...
class sold_itemsSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.hemnet.se/salda/bostader?location_ids%5B%5D=925968&location_ids%5B%5D=925970&location_ids%5B%5D=925969&location_ids%5B%5D=898472&item_types%5B%5D=bostadsratt&living_area_min=40&sold_age=3m"]
    #start_urls = ["https://www.hemnet.se/salda/bostader?location_ids%5B%5D=898741&item_types%5B%5D=bostadsratt"]
    counter = 0
    results = {}
    ...
    def parse(self, response):
        for ad in response.css("ul.sold-results > li.sold-results__normal-hit > a.sold-property-listing::attr('href')"):
            time.sleep(1.5)
            #print(ad.text())
...
def first_value_app(url):
    """
    Extract data price, price square meter, bild year, feed all in SEK
    url: link to uniq sold object
    box: specefy place of where to find data on the sidte
    label: specfy what is the label, ex 'Begärt pris'
    value: specfy what is the value, ex '2600000'
    """
    gubb = requests.get(url)
    drasl = BeautifulSoup(gubb.content, 'html.parser')
    first_box = drasl.find('dl', class_='sold-property__price-stats')
    label_first = first_box.find_all('dt', class_='sold-property__attribute')
    value_first = first_box.find_all('dd', class_='sold-property__attribute-value')

    for label,value in zip(label_first,value_first):
        #Extract label nice text
        label = label.replace(u"\n", "")
        label = label.replace(u"\n", "")
        label = label.replace(u"\t", "")
        label = label.replace(u'\xa0', '')

        #Extract value nice value
